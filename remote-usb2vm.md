...
2. **Load Kernel Modules:** Load the modules required to host/share a USB device.
   Bash
   sudo modprobe usbip\_core
   sudo modprobe usbip\_host
...
5. **Bind the Device for Sharing:** Use the busid from the previous step to make the device available for sharing over the network.
   Bash
...
2. **Load Kernel Module:** Load the virtual host controller module.6

   Bash
   sudo modprobe vhci-hcd

3. **List Remote Devices:** Probe your client machine's IP address to see the shared USB devices.
   Bash
   usbip list \--remote=\<IP\_OF\_YOUR\_GENTOO\_CLIENT\>

4. **Attach the Device:** Use the IP address and the busid to attach the remote device to your VM.7

   Bash
   sudo usbip attach \--remote=\<IP\_OF\_YOUR\_GENTOO\_CLIENT\> \--busid=1-4
